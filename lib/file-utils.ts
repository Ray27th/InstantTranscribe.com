...
export const SUPPORTED_AUDIO_TYPES = [
  ...
  'audio/mp4',
  'audio/m4a',
  'audio/x-m4a', // Some browsers report M4A files with this MIME type
  'audio/aac',
  'audio/ogg',
  'audio/webm',
  'audio/flac',
...
export const calculateCost = (durationInMinutes: number): number => {
  ...
  const roundedMinutes = Math.ceil(durationInMinutes * 10) / 10; // Rounds to nearest 0.1 minutes
...
// Format confidence as a clean percentage
export const formatConfidence = (confidence: number): string => {
  const percentage = confidence * 100;
  return `${Math.round(percentage)}%`;
};

// Format duration for display with proper decimals
export const formatDurationClean = (minutes: number): string => {
  if (minutes < 1) {
    const seconds = Math.round(minutes * 60);
    return `${seconds} second${seconds !== 1 ? 's' : ''}`;
  }
  
  // Round to 1 decimal place for professional display
  const roundedMinutes = Math.round(minutes * 10) / 10;
  
  if (roundedMinutes < 60) {
    if (roundedMinutes % 1 === 0) {
      return `${Math.round(roundedMinutes)} minute${Math.round(roundedMinutes) !== 1 ? 's' : ''}`;
    } else {
...
// Format processing time professionally
...
export const formatDuration = (minutes: number): string => {
  ...
  if (displaySeconds === 0) {
    ...
    return `${hours}h ${remainingMinutes}m ${displaySeconds}s`;
  }
};
...
---
  Generated by InstantTranscribe.com
Visit: https://instanttranscribe.com
`;
